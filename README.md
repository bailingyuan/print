...
**症状**：发送命令后15秒超时，日志显示 `Response timeout`

**原因**：**机号配置不正确** - 这是最常见的问题！

喷码机通过机号来识别命令。如果机号不匹配，喷码机会完全忽略所有命令。

**解决方法**：
1. 确认喷码机IP地址（如：169.254.59.119）
2. 机号 = IP地址最后一个字节（如：119）
3. 在设置面板中配置正确的机号
4. 保存设置并重新连接

**验证方法**：
- 查看通信日志中"发送"部分
- 检查"机号"字段：应显示 `77`（119的十六进制）
- 如果显示 `00`，说明机号配置未生效
